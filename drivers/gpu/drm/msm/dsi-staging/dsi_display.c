		...
		int power_mode, void *disp)
{
	...
	if (!display || !display->panel) {
        ...
        if (!connector || !connector->dev) {
                pr_debug_once("invalid connector/dev\n");
                return -EINVAL;
        } else {
                dev = connector->dev;
...
int dsi_display_enable(struct dsi_display *display)
{
	...
	if (display->is_cont_splash_enabled) {
		...
		if (panel->elvss_dimming_check_enable) {
			...
			if (rc) {
				dsi_panel_release_panel_lock(display->panel);
				pr_debug_once("Write elvss_dimming_offset cmds failed, rc=%d\n", rc);
				return 0;
			}
			...
			if (rc <= 0) {
				dsi_panel_release_panel_lock(display->panel);
				pr_debug_once("Read elvss_dimming_cmds failed, rc=%d\n", rc);
				return 0;
			}
		...
		if (display->panel->is_tddi_flag) {
			...
			if (!rc) {
				pr_debug_once("[%s] failed to read lockdowninfo para, rc=%d\n",
					display->name, rc);
			}
		}
		dsi_panel_release_panel_lock(display->panel);
			...
			bool enable)
{
	...
	if (!display) {
		pr_debug_once("Invalid display ptr\n");
		return -EINVAL;
	}
	...
	if (ret)
		pr_debug_once("[%s] failed to set esd irq, rc=%d\n",
				display->name, ret);
