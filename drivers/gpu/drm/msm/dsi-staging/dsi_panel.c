...
int dsi_panel_set_doze_backlight(struct dsi_display *display)
{
	...
	if (!dsi_display || !dsi_display->panel || !dsi_display->drm_dev) {
		pr_debug_once("invalid display/panel/drm_dev\n");
		return -EINVAL;
	}
	...
	if (drm_dev && (drm_dev->doze_state == MSM_DRM_BLANK_LP1 || drm_dev->doze_state == MSM_DRM_BLANK_LP2)) {
		...
		if (drm_dev->doze_brightness == DOZE_BRIGHTNESS_HBM) {
			...
			if (rc)
				pr_debug_once("[%s] failed to send DSI_CMD_SET_DOZE_HBM cmd, rc=%d\n", panel->name, rc);
			else
			...
			if (rc)
				pr_debug_once("[%s] failed to send DSI_CMD_SET_DOZE_LBM cmd, rc=%d\n",
				panel->name, rc);
			else
				pr_info("In %s the doze_brightness value:%u\n", __func__, drm_dev->doze_brightness);
			panel->in_aod = true;
...
int dsi_panel_parse_elvss_dimming_read_configs(struct dsi_panel *panel)
{
	...
	if (!panel->elvss_dimming_offset.count) {
		pr_debug_once("elvss dimming offset command parsing failed\n");
		return -EINVAL;
	}
	...
	if (!elvss_dimming_cmds->read_cmd.count) {
		pr_debug_once("elvss dimming command parsing failed\n");
		return -EINVAL;
	}
	...
	if (rc) {
		pr_debug_once("failed to parse elvss dimming read length, rc=%d\n", rc);
		return -EINVAL;
	}
	...
	if (!panel->hbm_fod_on.count) {
		pr_debug_once("hbm fod on command parsing failed\n");
		return -EINVAL;
	}
	...
	if (!panel->hbm_fod_off.count) {
		pr_debug_once("hbm fod off command parsing failed\n");
		return -EINVAL;
	}
	...
	if (!panel->hbm_fod_off_doze_hbm_on.count) {
		pr_debug_once("hbm fod off doze hbm on command parsing failed\n");
		return -EINVAL;
	}
	...
	if (!panel->hbm_fod_off_doze_hbm_on.count) {
		pr_debug_once("hbm fod off doze lbm on command parsing failed\n");
		return -EINVAL;
	}
	return 0;
}
...
static int dsi_panel_parse_esd_config(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(esd_config->esd_err_irq_gpio)) {
		...
		if (rc)
			pr_debug_once("%s: Failed to get esd irq gpio %d (code: %d)",
				  ...
				  bool enable)
{
	...
	if (!panel || !panel->panel_initialized) {
		pr_debug_once("[LCD] panel not ready!\n");
		return -EINVAL;
	}
				...
				int topology_override)
{
	...
	if (rc)
		pr_debug_once("failed to parse mi config, rc=%d\n", rc);
				...
				struct dsi_panel_cmd_set *cmd_sets)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (len < 0) {
			rc = len;
			pr_debug_once("failed to set cmds, rc=%d\n", rc);
			goto error;
		}
		if (cmds->post_wait_ms)
			usleep_range(cmds->post_wait_ms*1000,
...
error:
	if (panel->host_config.phy_type == DSI_PHY_TYPE_CPHY) {
		...
		if (rc) {
			pr_debug_once("[%s] failed to update TP fps code setting, rc=%d\n",
				panel->name, rc);
		}
	}
	mutex_unlock(&panel->panel_lock);
				...
				struct dsi_display_mode *adj_mode)
{
	...
	if (rc) {
		pr_debug_once("Failed to send DSI_CMD_SET_DISP_PEN_120HZ command\n");
		retval = -EAGAIN;
		goto error;
	}else
	...
	if (match && plockdowninfo) {
		...
		if (sscanf(lockdown_str, "0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x",
				...
				&plockdowninfo[4], &plockdowninfo[5], &plockdowninfo[6], &plockdowninfo[7])
					!= 8) {
			pr_debug_once("failed to parse lockdown info from cmdline !\n");
		} else {
...
int dsi_panel_lockdowninfo_param_read(struct dsi_panel *panel)
{
	...
	if (cmd_sets->cmds) {
		...
		if (rc) {
			pr_debug_once("[%s] failed to send cmds, rc=%d\n",panel->name, rc);
			rc = -EIO;
		}
		...
		if (strcmp(panel->name,"xiaomi 42 02 0a video mode dual dsi cphy panel") &&
			...
			if (rc <= 0) {
				pr_debug_once("[%s] failed to read cmds, rc=%d\n", panel->name, rc);
				rc = -EIO;
			}
			for(i = 0; i < 8; i++) {
			...
			for(i = 0; i < 8; i++) {
				...
				rc = dsi_panel_read_cmd_set(panel, &ld_read_config);
				if (rc <= 0) {
					pr_debug_once("[%s] failed to read, rc=%d\n", panel->name, rc);
					rc = -EIO;
				}
...
static ssize_t dsi_panel_xy_coordinate_read(struct dsi_panel *panel)
{
	...
	if (panel->cur_mode->priv_info->cmd_sets[DSI_CMD_SET_READ_XY_COORDINATE].cmds) {
		...
		if (rc) {
			pr_debug_once("[%s][%s] failed to send cmds, rc=%d\n", __func__, panel->name, rc);
			rc = -EIO;
			return rc;
		}
		...
		if (rc > 0)
			...
			pr_debug_once("[%s][%s] failed to read cmds, rc=%d\n", __func__, panel->name, rc);
	}
	return rc;
}
				...
				struct dsi_read_config *read_config)
{
	...
	if (count == 0) {
		pr_debug_once("No commands to be sent\n");
		goto exit;
	}
	if (cmds->last_command) {
		cmds->msg.flags |= MIPI_DSI_MSG_LASTCOMMAND;
