						...
						struct msm_dsi_host *msm_host)
{
	...
	if (IS_ERR(gdsc_reg)) {
		pr_debug_once("%s: cannot get gdsc\n", __func__);
		goto exit;
	}

	ahb_clk = clk_get(dev, "iface_clk");
	if (IS_ERR(ahb_clk)) {
		pr_debug_once("%s: cannot get interface clock\n", __func__);
		goto put_gdsc;
	}
	...
	if (ret) {
		pr_debug_once("%s: unable to enable gdsc\n", __func__);
		goto put_clk;
	}

	ret = clk_prepare_enable(ahb_clk);
	if (ret) {
		pr_debug_once("%s: unable to enable ahb_clk\n", __func__);
		goto disable_gdsc;
	}

	ret = dsi_get_version(msm_host->ctrl_base, &major, &minor);
	if (ret) {
		pr_debug_once("%s: Invalid version\n", __func__);
		goto disable_clks;
	}
...
static int dsi_host_regulator_enable(struct msm_dsi_host *msm_host)
{
	...
	for (i = 0; i < num; i++) {
		if (regs[i].enable_load >= 0) {
			ret = regulator_set_load(s[i].consumer,
						 regs[i].enable_load);
			if (ret < 0) {
				pr_debug_once("regulator %d set op mode failed, %d\n",
					i, ret);
				goto fail;
			}
		}
	...
	if (ret < 0) {
		pr_debug_once("regulator enable failed, %d\n", ret);
		goto fail;
	}
...
static int dsi_regulator_init(struct msm_dsi_host *msm_host)
{
	...
	if (ret < 0) {
		pr_debug_once("%s: failed to init regulator, ret=%d\n",
						__func__, ret);
		return ret;
	}
...
static int dsi_clk_init(struct msm_dsi_host *msm_host)
{
	...
	for (i = 0; i < cfg->num_bus_clks; i++) {
		...
		if (IS_ERR(msm_host->bus_clks[i])) {
			...
			pr_debug_once("%s: Unable to get %s, ret = %d\n",
	...
	if (IS_ERR(msm_host->byte_clk)) {
		...
		pr_debug_once("%s: can't find dsi_byte_clk. ret=%d\n",
			__func__, ret);
	...
	if (IS_ERR(msm_host->pixel_clk)) {
		...
		pr_debug_once("%s: can't find dsi_pixel_clk. ret=%d\n",
			__func__, ret);
	...
	if (IS_ERR(msm_host->esc_clk)) {
		...
		pr_debug_once("%s: can't find dsi_esc_clk. ret=%d\n",
			__func__, ret);
	...
	if (!msm_host->byte_clk_src) {
		ret = -ENODEV;
		pr_debug_once("%s: can't find byte_clk_src. ret=%d\n", __func__, ret);
		goto exit;
	}
	...
	if (!msm_host->pixel_clk_src) {
		ret = -ENODEV;
		pr_debug_once("%s: can't find pixel_clk_src. ret=%d\n", __func__, ret);
		goto exit;
	}
	...
	if (cfg_hnd->major == MSM_DSI_VER_MAJOR_V2) {
		...
		if (IS_ERR(msm_host->src_clk)) {
			...
			pr_debug_once("%s: can't find dsi_src_clk. ret=%d\n",
				__func__, ret);
		...
		if (!msm_host->esc_clk_src) {
			ret = -ENODEV;
			pr_debug_once("%s: can't get esc_clk_src. ret=%d\n",
				__func__, ret);
			goto exit;
		}
		...
		if (!msm_host->dsi_clk_src) {
			ret = -ENODEV;
			pr_debug_once("%s: can't get dsi_clk_src. ret=%d\n",
				__func__, ret);
		}
	}
exit:
...
static int dsi_bus_clk_enable(struct msm_dsi_host *msm_host)
{
	...
	for (i = 0; i < cfg->num_bus_clks; i++) {
		...
		if (ret) {
			pr_debug_once("%s: failed to enable bus clock %d ret %d\n",
				__func__, i, ret);
			goto err;
		}
	}
...
static int dsi_link_clk_enable_6g(struct msm_dsi_host *msm_host)
{
	...
	if (ret) {
		pr_debug_once("%s: Failed to set rate byte clk, %d\n", __func__, ret);
		goto error;
	}

	ret = clk_set_rate(msm_host->pixel_clk, msm_host->mode->clock * 1000);
	if (ret) {
		pr_debug_once("%s: Failed to set rate pixel clk, %d\n", __func__, ret);
		goto error;
	}

	ret = clk_prepare_enable(msm_host->esc_clk);
	if (ret) {
		pr_debug_once("%s: Failed to enable dsi esc clk\n", __func__);
		goto error;
	}

	ret = clk_prepare_enable(msm_host->byte_clk);
	if (ret) {
		pr_debug_once("%s: Failed to enable dsi byte clk\n", __func__);
		goto byte_clk_err;
	}

	ret = clk_prepare_enable(msm_host->pixel_clk);
	if (ret) {
		pr_debug_once("%s: Failed to enable dsi pixel clk\n", __func__);
		goto pixel_clk_err;
	}
...
static int dsi_link_clk_enable_v2(struct msm_dsi_host *msm_host)
{
	...
	if (ret) {
		pr_debug_once("%s: Failed to set rate esc clk, %d\n", __func__, ret);
		goto error;
	}

	ret = clk_set_rate(msm_host->src_clk, msm_host->src_clk_rate);
	if (ret) {
		pr_debug_once("%s: Failed to set rate src clk, %d\n", __func__, ret);
		goto error;
	}
	...
	if (ret) {
		pr_debug_once("%s: Failed to enable dsi src clk\n", __func__);
		goto src_clk_err;
	}
...
static int dsi_calc_clk_rate(struct msm_dsi_host *msm_host)
{
	...
	if (lanes > 0) {
		...
		pr_debug_once("%s: forcing mdss_dsi lanes to 1\n", __func__);
...
static int dsi_tx_buf_alloc(struct msm_dsi_host *msm_host, int size)
{
	...
	if (cfg_hnd->major == MSM_DSI_VER_MAJOR_6G) {
		...
		if (IS_ERR(msm_host->tx_gem_obj)) {
			...
			pr_debug_once("%s: failed to allocate gem, %d\n",
				__func__, ret);
		...
		if (ret) {
			pr_debug_once("%s: failed to get iova, %d\n", __func__, ret);
			return ret;
		}

		if (iova & 0x07) {
			pr_debug_once("%s: buf NOT 8 bytes aligned\n", __func__);
			return -EINVAL;
		}
		...
		if (!msm_host->tx_buf) {
			ret = -ENOMEM;
			pr_debug_once("%s: failed to allocate tx buf, %d\n",
				__func__, ret);
			return ret;
		}
			   ...
			   const struct mipi_dsi_msg *msg)
{
	...
	if (ret) {
		pr_debug_once("%s: create packet failed, %d\n", __func__, ret);
		return ret;
	}
	len = (packet.size + 3) & (~0x3);

	if (len > msm_host->tx_size) {
		pr_debug_once("%s: packet size is too big\n", __func__);
		return -EINVAL;
	}
	...
	if (cfg_hnd->major == MSM_DSI_VER_MAJOR_6G) {
		...
		if (IS_ERR(data)) {
			ret = PTR_ERR(data);
			pr_debug_once("%s: get vaddr failed, %d\n", __func__, ret);
			return ret;
		}
	} else {
...
static int dsi_short_read1_resp(u8 *buf, const struct mipi_dsi_msg *msg)
{
	...
	if (data && (msg->rx_len >= 1)) {
		*data = buf[1]; /* strip out dcs type */
		...
		pr_debug_once("%s: read data does not match with rx_buf len %zu\n",
			__func__, msg->rx_len);
		return -EINVAL;
	}
}
				...
				const struct mipi_dsi_msg *msg)
{
	...
	if (!len) {
		pr_debug_once("%s: failed to add cmd type = 0x%x\n",
			__func__,  msg->type);
		return -EINVAL;
	}
	...
	*/
	...
	if ((msm_host->mode_flags & MIPI_DSI_MODE_VIDEO) && (len > bllp_len)) {
		pr_debug_once("%s: cmd cannot fit into BLLP period, len=%d\n",
			__func__, len);
		return -EINVAL;
	}
	...
	if (ret < len) {
		pr_debug_once("%s: cmd dma tx failed, type=0x%x, data0=0x%x, len=%d\n",
...
int msm_dsi_host_init(struct msm_dsi *msm_dsi)
{
	...
	if (!msm_host) {
		pr_debug_once("%s: FAILED: cannot alloc dsi host\n",
		       __func__);
		ret = -ENOMEM;
		goto fail;
	}
	...
	if (ret) {
		pr_debug_once("%s: failed to parse dt\n", __func__);
		goto fail;
	}

	msm_host->ctrl_base = msm_ioremap(pdev, "dsi_ctrl", "DSI CTRL");
	if (IS_ERR(msm_host->ctrl_base)) {
		pr_debug_once("%s: unable to map Dsi ctrl base\n", __func__);
	...
	if (!msm_host->cfg_hnd) {
		ret = -EINVAL;
		pr_debug_once("%s: get config failed\n", __func__);
		goto fail;
	}
	...
	if (msm_host->id < 0) {
		ret = msm_host->id;
		pr_debug_once("%s: unable to identify DSI host index\n", __func__);
		goto fail;
	}
	...
	if (ret) {
		pr_debug_once("%s: regulator init failed\n", __func__);
		goto fail;
	}

	ret = dsi_clk_init(msm_host);
	if (ret) {
		pr_debug_once("%s: unable to initialize dsi clks\n", __func__);
		goto fail;
	}
	...
	if (!msm_host->rx_buf) {
		ret = -ENOMEM;
		pr_debug_once("%s: alloc rx temp buf failed\n", __func__);
		goto fail;
	}
					...
					struct drm_device *dev)
{
	...
	if (ret) {
		pr_debug_once("%s: alloc tx gem obj failed, %d\n", __func__, ret);
		return ret;
	}
				...
				const struct mipi_dsi_msg *msg)
{
	...
	while (!end) {
		...
		if (ret < 2) {
			pr_debug_once("%s: Set max pkt size failed, %d\n",
				__func__, ret);
			return -EINVAL;
		}
		...
		if (ret < msg->tx_len) {
			pr_debug_once("%s: Read cmd Tx failed, %d\n", __func__, ret);
			return ret;
		}
	...
	case MIPI_DSI_RX_ACKNOWLEDGE_AND_ERROR_REPORT:
		pr_debug_once("%s: rx ACK_ERR_PACLAGE\n", __func__);
		ret = 0;
		break;
	...
	struct msm_dsi_pll *src_pll)
{
	...
	if (ret) {
		pr_debug_once("%s: can't set parent to byte_clk_src. ret=%d\n",
			__func__, ret);
		goto exit;
	}
	...
	if (ret) {
		pr_debug_once("%s: can't set parent to pixel_clk_src. ret=%d\n",
			__func__, ret);
		goto exit;
	}
	...
	if (cfg_hnd->major == MSM_DSI_VER_MAJOR_V2) {
		...
		if (ret) {
			pr_debug_once("%s: can't set parent to dsi_clk_src. ret=%d\n",
				__func__, ret);
			goto exit;
		}
		...
		if (ret) {
			pr_debug_once("%s: can't set parent to esc_clk_src. ret=%d\n",
				__func__, ret);
			goto exit;
		}
	}
	...
	struct msm_dsi_phy_clk_request *clk_req)
{
	...
	if (ret) {
		pr_debug_once("%s: unable to calc clk rate, %d\n", __func__, ret);
		return;
	}
			...
			struct msm_dsi_phy_shared_timings *phy_shared_timings)
{
	...
	if (ret) {
		pr_debug_once("%s:Failed to enable vregs.ret=%d\n",
			__func__, ret);
		goto unlock_ret;
	}
	...
	if (ret) {
		pr_debug_once("%s: failed to enable link clocks. ret=%d\n",
		       __func__, ret);
	...
	if (ret) {
		pr_debug_once("%s: failed to set pinctrl default state, %d\n",
			__func__, ret);
					...
					struct drm_display_mode *mode)
{
	...
	if (!msm_host->mode) {
		pr_debug_once("%s: cannot duplicate mode\n", __func__);
		return -ENOMEM;
	}
