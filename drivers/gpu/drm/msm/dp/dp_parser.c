...
static int dp_parser_reg(struct dp_parser *parser)
{
	...
	for (i = 0; i < reg_count; i++) {
		...
		rc = msm_dss_ioremap_byname(pdev, &io->data[i].io,
			io->data[i].name);
		if (rc) {
			pr_debug_once("unable to remap %s resources\n",
				io->data[i].name);
			goto err;
		}
	}
...
static int dp_parser_aux(struct dp_parser *parser)
{
	...
	for (i = 0; i < PHY_AUX_CFG_MAX; i++) {
		...
		if (!data) {
			pr_debug_once("Unable to read %s\n", property);
			goto error;
		}
		...
		if ((config_count < minimum_config_count) ||
			...
			pr_debug_once("Invalid config count (%d) configs for %s\n",
		...
		enum dp_pm_type module)
{
	...
	if (!supply_root_node) {
		pr_debug_once("no supply entry present: %s\n", pm_supply_name);
		goto novreg;
	}
	...
	for_each_child_of_node(supply_root_node, supply_node) {
		...
		rc = of_property_read_string(supply_node,
			"qcom,supply-name", &st);
		if (rc) {
			pr_debug_once("error reading name. rc=%d\n",
				 rc);
			goto error;
		}
		...
		if (rc) {
			pr_debug_once("error reading min volt. rc=%d\n",
				rc);
			goto error;
		}
		...
		if (rc) {
			pr_debug_once("error reading max volt. rc=%d\n",
				rc);
			goto error;
		}
		...
		if (rc) {
			pr_debug_once("error reading enable load. rc=%d\n",
				rc);
			goto error;
		}
		...
		if (rc) {
			pr_debug_once("error reading disable load. rc=%d\n",
				rc);
			goto error;
		}
...
static int dp_parser_regulator(struct dp_parser *parser)
{
	...
	for (i = DP_CORE_PM; i <= DP_PHY_PM; i++) {
		...
		if (rc) {
			pr_debug_once("get_dt_vreg_data failed for %s. rc=%d\n",
...
static int dp_parser_init_clk_data(struct dp_parser *parser)
{
	...
	if (num_clk <= 0) {
		pr_debug_once("no clocks are defined\n");
		rc = -EINVAL;
		goto exit;
	}
	...
	if (core_clk_count <= 0) {
		pr_debug_once("no core clocks are defined\n");
		rc = -EINVAL;
		goto exit;
	}
	...
	if (link_clk_count <= 0) {
		pr_debug_once("no link clocks are defined\n");
		rc = -EINVAL;
...
static int dp_parser_clock(struct dp_parser *parser)
{
	...
	if (rc) {
		pr_debug_once("failed to initialize power data\n");
		rc = -EINVAL;
		goto exit;
	}
...
void dp_parser_put(struct dp_parser *parser)
{
	...
	if (!parser) {
		pr_debug_once("invalid parser module\n");
		return;
	}
